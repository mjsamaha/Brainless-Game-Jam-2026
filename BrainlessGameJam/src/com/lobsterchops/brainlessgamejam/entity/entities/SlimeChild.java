...
public class SlimeChild extends Entity {
	...
	private static final Map<Direction, BufferedImage> SPRITES = Map.of(Direction.DOWN,
			AssetManager.get(Gfx.SLIME_CHILD_DOWN, DRAW_WIDTH, DRAW_HEIGHT), Direction.UP,
			AssetManager.get(Gfx.SLIME_CHILD_UP, DRAW_WIDTH, DRAW_HEIGHT), Direction.LEFT,
			AssetManager.get(Gfx.SLIME_CHILD_LEFT, DRAW_WIDTH, DRAW_HEIGHT), Direction.RIGHT,
			AssetManager.get(Gfx.SLIME_CHILD_RIGHT, DRAW_WIDTH, DRAW_HEIGHT));
	...
	public SlimeChild(int index, LinkedList<Vector2> positionHistory) {
		super(positionHistory.isEmpty() ? Vector2.ZERO
				: positionHistory.get(Math.min((index + 1) * DELAY, positionHistory.size() - 1)), SIZE, SIZE);
	...
	/**
	 * If standing in water with no log, publish EntityDestroyed. PlayingScene's
	 * loseLastChild() handles which child actually disappears.
	 */
