...
public class SlimeParent extends Entity {
	...
	private static final Map<Direction, BufferedImage> SPRITES = Map.of(Direction.DOWN,
			AssetManager.get(Gfx.SLIME_PARENT_DOWN, DRAW_WIDTH, DRAW_HEIGHT), Direction.UP,
			AssetManager.get(Gfx.SLIME_PARENT_UP, DRAW_WIDTH, DRAW_HEIGHT), Direction.LEFT,
			AssetManager.get(Gfx.SLIME_PARENT_LEFT, DRAW_WIDTH, DRAW_HEIGHT), Direction.RIGHT,
			AssetManager.get(Gfx.SLIME_PARENT_RIGHT, DRAW_WIDTH, DRAW_HEIGHT));
	...
	/**
	 * Prevents CrossingCompleted from firing more than once per wave. Reset by
	 * WaveManager via {@link #resetCrossingGuard()} when the parent is repositioned
	 * for the next wave.
	 */
	...
	public void render(Graphics2D g2) {
		...
		if (sprite != null) {
			g2.drawImage(sprite, (int) getPosition().x() - DRAW_WIDTH / 2, (int) getPosition().y() - DRAW_HEIGHT / 2,
					null);
		}
	}
	...
	/**
	 * Fires {@link CrossingCompleted} once when the parent reaches the top row. The
	 * {@code allAlive} flag compares living children against the wave's starting
	 * count — supplied by counting SlimeChild objects in GameSystem.
	 */
	private void checkCrossing(UpdateContext context) {
		if (hasCrossed)
			return;
		if (getPosition().y() >= CROSSING_THRESHOLD_Y)
			return;
