...
public class RiverLayout {
	...
	public void clearLogs(GameSystem gameSystem) {
		gameSystem.getObjects().stream().filter(o -> o instanceof Log).map(o -> (Log) o)
				.forEach(log -> log.markInactive());
	}

	/**
	 * Returns the world-space Y bounds (top, bottom) of every contiguous water zone
	 * in the map. Used by entities to check if they're standing in water.
	 */
